...
const Nosotros = () => {
    ...
    const getIcon = (iconName) => {
        const icons = {
            'diseño-interior': <FaPalette className="text-2xl text-primary" />,
            'letreros-publicitarios': <FaLightbulb className="text-2xl text-primary" />,
            'iluminacion-led': <FaLightbulb className="text-2xl text-primary" />,
            'estructuras-metalicas': <FaIndustry className="text-2xl text-primary" />,
            'mantenimiento': <FaClock className="text-2xl text-primary" />,
            'default': <FaStar className="text-2xl text-primary" />
        };
        return icons[iconName] || icons.default;
    };

    // Función para obtener el slug del servicio
    const getServiceSlug = (servicioTitulo) => {
        const slugMap = {
            'Diseño de Interiores': 'diseno-interiores',
            'Letreros Publicitarios': 'letreros-publicitarios',
            'Iluminación LED y Neón': 'iluminacion-led',
            'Estructuras Metálicas a Medida': 'estructuras-metalicas',
            'Mantenimiento Especializado': 'mantenimiento'
        };
        return slugMap[servicioTitulo] || servicioTitulo.toLowerCase().replace(/\s+/g, '-');
    };
